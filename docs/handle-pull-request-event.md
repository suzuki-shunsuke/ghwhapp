...
Previously, ghwhapp only subscribed to Pull Request Review Events.
Starting with v0.3.2, it can also subscribe to Pull Request Events.
Because only Pull Request Review Events were subscribed to before, pushing empty commits or trivial merge commits to an already-approved PR would not create a ghwhapp check on the pushed commit, requiring an additional approval and degrading the developer experience.
By subscribing to Pull Request Events, ghwhapp creates a check on the pushed commit without requiring an additional approval.
ghwhapp only handles the `synchronize` action of Pull Request Events and ignores all other actions.
...
If the target commit is an empty commit or a trivial merge commit, ghwhapp walks back through the parent commits on the PR's head branch using the same logic, and creates a check on the target commit.
...
ghwhapp again walks back through the commits to commit 1, and marks commit 3 as success.
...
ghwhapp checks commit 4, but since it is neither a trivial merge commit nor an empty commit, no check is created.
Review is required.
...
ghwhapp walks back through the commits to commit 4, but since commit 4 is not approved and is neither a trivial merge commit nor an empty commit, no check is created.
